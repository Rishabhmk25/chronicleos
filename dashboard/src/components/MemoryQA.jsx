...
export default function MemoryQA() {
  ...
  const submit = async (overrideQuery) => {
    ...
    try {
      ...
      if (mode === "ask") {
        r = await axios.get(
          `/ask?q=${encodeURIComponent(q)}&level=${detailLevel}`,
        );
      ...
      const data = r.data;
      const errorMsg = data.answer?.startsWith("Error:")
        ? data.answer
        : data.narrative?.startsWith("Error:")
          ? data.narrative
      ...
      if (errorMsg) {
        if (
          errorMsg.includes("Number of requested results 0") ||
          errorMsg.includes("empty")
        ) {
          setError(
            "No matching memories found. Either your database is empty, or there's no captured data related to this topic yet.",
          );
        } else {
      ...
      <div className="section-header">
        <div>
          ...
          <p className="section-subtitle">
            Ask anything about your browsing history — powered by RAG + Groq
            llama3
          </p>
        </div>
      </div>
      ...
      {mode === "ask" && (
        ...
        >
          ...
          >
            <option
              value="lite"
              style={{ background: "black", color: "var(--text-primary)" }}
            >
              Lite (Fast & Concise)
            </option>
            <option
                    ...
                    {s.hybrid_score != null && (
                      <div className="score-bar">
                        <div
                          ...
                          style={{
                            width: `${(s.hybrid_score * 100).toFixed(0)}%`,
                          }}
                        />
                      </div>
                    )}
