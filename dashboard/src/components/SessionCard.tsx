...
function sessionEmoji(label: string): string {
  const l = label.toLowerCase()
  if (l.includes("code") || l.includes("program") || l.includes("react") || l.includes("python")) return "◻"
  if (l.includes("research") || l.includes("paper") || l.includes("study"))  return "◈"
  if (l.includes("video") || l.includes("youtube") || l.includes("watch"))    return "▷"
  if (l.includes("news") || l.includes("article"))                             return "◉"
  if (l.includes("design") || l.includes("ui") || l.includes("css"))          return "◆"
  if (l.includes("ai") || l.includes("ml") || l.includes("neural") || l.includes("llm")) return "⬡"
  if (l.includes("shop") || l.includes("buy") || l.includes("amazon"))        return "◎"
  if (l.includes("health") || l.includes("medical") || l.includes("drug"))    return "◇"
  return "◈"
}

export default function SessionCard({ session }: { session: Session }) {
  const [expanded, setExpanded] = useState(false)
  const [pages, setPages] = useState<Page[]>(session.pages || [])
    ...
    <div className={`session-card ${expanded ? "expanded" : ""}`}>
      <div className="session-header" onClick={() => setExpanded(e => !e)}>
        <div className="session-icon">
          {sessionEmoji(session.label || "")}
        </div>
        <div className="session-info">
          <div className="session-label">{session.label || "Unnamed Session"}</div>
          ...
          {pages.map((page, i) => (
            <div key={i} className="page-item">
              ...
              <span className="page-time">
                {page.timestamp ? format(new Date(page.timestamp), "h:mm a") : ""}
              </span>
              <button 
                onClick={(e) => { e.preventDefault(); e.stopPropagation(); deleteCapture(page.id); }}
                style={{ background: "none", border: "none", color: "var(--crimson)", cursor: "pointer", opacity: 0.6, fontSize: 14, marginLeft: 12 }}
      ...
      {expanded && (!pages || pages.length === 0) && (
        <div className="session-pages">
          <div className="page-item" style={{ color: "var(--text-ghost)", justifyContent: "center" }}>
