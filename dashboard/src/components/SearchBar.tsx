...
export default function SearchBar() {
  ...
  const exampleQueries = sessions.length > 0
  ...
  return (
    <div>
      ...
      <div className="search-bar">
        ...
        >
          {loading
            ? <><span className="spinner" style={{ borderTopColor: "#fff" }} /> Searching...</>
            : "⌕ Search"
          }
        </button>
      </div>
      ...
      {searched && !loading && results.length === 0 && !error && (
        <div className="empty-state">
          ...
          <div className="empty-state-desc">
            Try a different query, or make sure the backend has indexed your captures.
          </div>
        </div>
      )}
      ...
      {results.length > 0 && (
        <>
          <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 14 }}>
            <span style={{ color: "var(--ice)", fontFamily: "'Geist Mono', monospace" }}>
              {results.length} result{results.length !== 1 ? "s" : ""}
            </span>
            {" "}for{" "}
            <span style={{ color: "var(--crimson)" }}>"{query}"</span>
            {elapsed != null && (
              <span
                style={{
                  marginLeft: 10,
              ...
              <div key={result.id} className="result-card">
                ...
                <div className="result-meta">
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <button 
                      onClick={(e) => { e.preventDefault(); e.stopPropagation(); deleteCapture(result.id); }}
                      style={{ background: "none", border: "none", color: "var(--crimson)", cursor: "pointer", opacity: 0.6, fontSize: 14 }}
                  ...
                  {result.timestamp && (
                    <span style={{ fontSize: 11, color: "var(--text-ghost)", fontFamily: "'Geist Mono', monospace" }}>
                ...
                <div className="score-bar" style={{ marginTop: 10 }}>
                  <div
                    ...
                    style={{ width: `${(result.hybrid_score * 100).toFixed(0)}%` }}
                  />
                </div>
