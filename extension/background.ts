      ...
      try {
        ...
        if (results && results[0] && results[0].result) {
          ...
          if (Object.keys(updates).length > 0) {
            ...
            console.log("ChronicleOS: Background auto-synced auth token & BYOK config from dashboard tab!")
          }
        }
      } catch (e) {
...
async function queueVisit(tab: chrome.tabs.Tab, tabId: number) {
  ...
  console.log(`ChronicleOS queued page capture: ${tab.title}. Queue length: ${captureQueue.length}`)
...
async function flushQueue() {
  ...
  try {
    const { cos_token, cos_groq_api_key, cos_nomic_api_key } = await chrome.storage.local.get(["cos_token", "cos_groq_api_key", "cos_nomic_api_key"])
    ...
    console.log(`ChronicleOS syncing batch of ${batch.length} pages to ${backendUrl}...`)
    ...
    console.log(`ChronicleOS successfully synced batch of ${batch.length} pages.`)
  } catch (e) {
